                          ...
                          )
{
  ...
  for (j = j0; j < j1; j += step_v)
  {
    ...
    for (i = i0; i < i1; i += step_h)
    {
      ...
      if(currMB->block_x == 84 && currMB->block_y == 68)
      {
          FILE  *f = fopen("MVD.txt", "a+");
                    fprintf(f,"\ncords :%i %i",currMB->block_x, currMB->block_y);
                    //fprintf(f,"\nMVX Vectors\n\t %i %i",cur_mv->mv_x,predMV.mv_x);
                    //fprintf(f,"\nMVY Vectors\n\t %i %i",cur_mv->mv_y,predMV.mv_y);
                    //fprintf(f,"\nMVD Vectors\n\t %i %i",mvd[0],mvd[1]);
                fclose(f);
                mvd[1] = -10;
                mvd[0] = -10;
